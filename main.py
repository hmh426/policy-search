...
def render_search_tab():
    ...
    # ── 검색 입력 ────────────────────────────────────────────────
    col1, col2, col3 = st.columns([4, 1, 1.8])
    ...
    tc1, tc2, tc3 = st.columns([4, 3, 3])
    tc1.markdown(f"**{len(results)}개 결과**")
    highlight_on = tc2.toggle("강조", value=True)
    expand_all = tc3.toggle("펼치기", value=True)
...
def render_help_tab():
    ...
    st.markdown("""
> 보험 약관 PDF는 **관리자가 미리 등록하고 인덱싱을 완료**한 상태입니다.
> 별도의 파일 업로드나 인덱싱 작업 없이 **바로 검색하시면 됩니다.**
...
**📁 문서 관리** 탭에서 현재 등록된 보험 상품과 문서 목록을 확인할 수 있습니다.

---
#### 3. 문의

새로운 보험 상품 추가가 필요하면 관리자에게 요청해주세요.
""")
